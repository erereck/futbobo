...
function currentCupRules(groupStage: boolean): BotaoRules {
  ...
  try {
    ...
    const halfSeconds =
      settings.botaoHalfSeconds ?? DEFAULT_BOTAO_RULES.halfSeconds;
    const extraSeconds =
      settings.botaoExtraSeconds ?? DEFAULT_BOTAO_RULES.extraSeconds;
    const penaltyRounds =
      settings.botaoPenaltyRounds ?? DEFAULT_BOTAO_RULES.penaltyRounds;
    return {
      ...base,
      goalLimit: [0, 3, 5].includes(goalLimit)
        ? goalLimit
        : DEFAULT_BOTAO_RULES.goalLimit,
      halfSeconds: [90, 120, 180].includes(halfSeconds)
        ? halfSeconds
        : DEFAULT_BOTAO_RULES.halfSeconds,
      extraSeconds: [30, 45, 60].includes(extraSeconds)
        ? extraSeconds
        : DEFAULT_BOTAO_RULES.extraSeconds,
      penaltyRounds: [3, 5].includes(penaltyRounds)
        ? penaltyRounds
        : DEFAULT_BOTAO_RULES.penaltyRounds,
    };
  } catch {
    return base;
  }
...
function tournamentField(country: Country, seed: number) {
  ...
  const ranked = COUNTRIES.filter((candidate) => candidate.id !== country.id)
    .map((candidate) => ({
      candidate,
      score: candidate.strength + rng.range(-2.8, 2.8),
    }))
...
function groupOpponents(
  ...
  for (const entry of candidates) {
    ...
    if (
      chosen.some(
        (candidate) => candidate.confederation === entry.country.confederation,
      ) &&
      entry.country.confederation === country.confederation
    )
      continue;
...
function knockoutOpponent(campaign: CupCampaign, stage: string) {
  ...
  const available = campaign.participants
    ...
    .filter(
      (country) =>
        country.id !== campaign.countryId &&
        !campaign.usedOpponents.includes(country.id),
    )
    .map((country) => ({
      country,
      score:
        Math.abs(country.strength - minimumStrength) +
        rng.range(0, stage === "Final" ? 4 : 10),
    }))
    .sort((a, b) => a.score - b.score);
  if (stage === "Final") {
    return (
      [...available].sort(
        (a, b) => b.country.strength - a.country.strength || a.score - b.score,
      )[0]?.country.id ?? available[0].country.id
    );
  }
  return (
    available[0]?.country.id ??
    campaign.participants.find((id) => id !== campaign.countryId)!
  );
}
...
function campaignTotals(campaign: CupCampaign) {
  return campaign.games.reduce(
    (totals, game) => {
      ...
      if (game.stage.startsWith("Grupo"))
        totals.points +=
          game.result.outcome === "win"
            ? 3
            : game.result.outcome === "draw"
              ? 1
              : 0;
      return totals;
    },
...
export default function WorldCupModePage() {
  ...
  function beginCup() {
    ...
    setCampaign({
      ...
      playerNumber: 10,
      position,
      games: opponents.map((opponentId, index) => ({
        stage: `Grupo · Rodada ${index + 1}`,
        opponentId,
      })),
      currentIndex: 0,
      participants,
  ...
  function registerResult(result: BotaoMatchResult) {
    ...
    setCampaign((current) => {
      ...
      const games = current.games.map((game, index) =>
        index === current.currentIndex ? { ...game, result } : game,
      );
  ...
  function advanceTournament() {
    ...
    if (groupStage) {
      ...
      const qualified =
        groupTotals.points >= 3 ||
        (groupTotals.points === 2 &&
          groupTotals.goalsFor >= groupTotals.goalsAgainst);
      if (!qualified) {
    ...
    const nextStage =
      KNOCKOUT_STAGES[KNOCKOUT_STAGES.indexOf(currentGame.stage) + 1];
  ...
  if (screen === "match" && setup) {
    return (
      <BotaoMatch key={setup.matchId} setup={setup} onFinish={registerResult} />
    );
  }
  ...
  if (screen === "setup") {
    return (
      <main className="world-cup-shell world-cup-setup">
        <header className="world-cup-brand">
          <Link href="/">← FUTBOBO</Link>
        ...
        <section className="world-cup-hero">
          ...
          <p>
            Comece na fase de grupos e jogue cada partida até a final. Nada
            altera sua carreira: aqui só existe o próximo jogo.
          </p>
        </section>
        <section className="world-cup-config">
          <div className="world-cup-config-heading">
            <span>01</span>
            <div>
              <small>SUA CAMISA</small>
              <h2>Escolha a Seleção</h2>
            </div>
          </div>
          <label className="world-cup-search">
            <span>BUSCAR SELEÇÃO</span>
            <input
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Brasil, Japão, Marrocos..."
            />
          </label>
        ...
        <section className="world-cup-config world-cup-role-config">
          ...
          <button
            className="world-cup-primary"
            type="button"
            onClick={beginCup}
          >
            Começar a Copa do Mundo <span>→</span>
          </button>
          <p className="world-cup-fineprint">
            48 seleções geradas · 3 jogos de grupo · mata-mata desde os 16 avos
            · campanha separada do modo carreira
          </p>
        </section>
      </main>
    );
    ...
    return (
      <main className="world-cup-shell world-cup-result">
        <header className="world-cup-brand">
          ...
          <b>{currentGame.stage}</b>
        </header>
        <section
          className={`world-cup-result-hero ${won ? "won" : draw ? "draw" : "lost"}`}
        >
          <small>{won ? "VITÓRIA" : draw ? "EMPATE" : "DERROTA"}</small>
          <div>
            <span>{nation.abbr}</span>
            <strong>
              {currentGame.result.goalsFor} × {currentGame.result.goalsAgainst}
            </strong>
            <span>{opponent.abbr}</span>
          </div>
          <p>
            {nation.name} × {opponent.name}
          </p>
        </section>
        <section className="world-cup-match-summary">
          <div>
            <small>GOLS SEUS</small>
            <strong>{currentGame.result.playerGoals}</strong>
          </div>
          <div>
            <small>ASSISTÊNCIAS</small>
            <strong>{currentGame.result.playerAssists}</strong>
          </div>
          <div>
            <small>TOQUES</small>
            <strong>{currentGame.result.turns}</strong>
          </div>
          <div>
            <small>CAMPANHA</small>
            <strong>
              {totals.wins}V {totals.draws}E {totals.losses}D
            </strong>
          </div>
        </section>
        <button
        ...
        >
          {currentGame.stage === "Final" ? "Ver desfecho" : "Continuar a Copa"}
          <span>→</span>
        </button>
      </main>
    );
  ...
  if (screen === "finished") {
    return (
      <main className="world-cup-shell world-cup-finished">
        <header className="world-cup-brand">
          <Link href="/">← FUTBOBO</Link>
          <span>COPA ENCERRADA</span>
        </header>
        <section
          className={`world-cup-finish-hero ${campaign.champion ? "champion" : "eliminated"}`}
        >
          <Image
            src={flagPath(nation.id)}
            alt=""
            width={96}
            height={68}
            unoptimized
          />
          <span>
            {campaign.champion ? "CAMPEÃO DO MUNDO" : "FIM DA CAMPANHA"}
          </span>
          <h1>
            {campaign.champion ? "A taça é sua." : `${nation.name} se despede.`}
          </h1>
          <p>
            {totals.played} jogos · {totals.wins} vitórias · {totals.goalsFor}{" "}
            gols marcados · {totals.goalsAgainst} sofridos
          </p>
        </section>
  ...
  return (
    <main className="world-cup-shell world-cup-hub">
      ...
      <section className="world-cup-score-rail">
        <div className="world-cup-nation">
          <Image
            src={flagPath(nation.id)}
            alt=""
            width={70}
        ...
        <div className="world-cup-live-metrics">
          <span>
            <small>J</small>
            <b>{totals.played}</b>
          </span>
          ...
          <span>
            <small>SG</small>
            <b>{totals.goalsFor - totals.goalsAgainst}</b>
          </span>
        </div>
      </section>
      <section className="world-cup-next-match">
        ...
        <div>
          <span>
            <Image
              ...
              unoptimized
            />
            <strong>{nation.abbr}</strong>
          </span>
          <b>×</b>
          <span>
            <Image
              src={flagPath(opponent.id)}
              alt=""
              width={58}
              height={40}
              unoptimized
        ...
        <h1>
          {nation.name} contra {opponent.name}
        </h1>
        <button
          className="world-cup-primary"
          type="button"
        ...
        >
          Jogar partida <span>→</span>
        </button>
      </section>
      <section className="world-cup-route">
        <div className="world-cup-route-heading">
          <small>TRILHA DA COPA</small>
          <strong>{campaign.currentIndex + 1} de até 8 partidas</strong>
        </div>
        <div className="world-cup-route-list">
          {campaign.games.map((game, index) => {
            ...
            const state = game.result
              ? game.result.outcome
              : index === campaign.currentIndex
                ? "current"
                : "future";
            return (
              <article
                key={`${game.stage}-${game.opponentId}`}
                className={`world-cup-route-game ${state}`}
              >
                <span>
                  {game.result
                    ? game.result.outcome === "win"
                ...
                <div>
                  ...
                  <strong>{gameOpponent.name}</strong>
                </div>
                {game.result ? (
                  <b>
          ...
          {KNOCKOUT_STAGES.slice(Math.max(0, campaign.games.length - 3)).map(
            (stage) => (
              <article key={stage} className="world-cup-route-game future">
                <span>·</span>
                <div>
