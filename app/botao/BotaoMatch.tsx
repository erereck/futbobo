...
import {
  chooseCpuPenaltyShot,
  chooseCpuShot,
  chooseCpuSubstitution,
  cpuSkillFor,
} from "./cpu";
import {
  advanceClock,
...
const DEFAULT_LOCAL_PLAYER_NAMES: Record<BotaoSide, string> = {
  user: "Jogador 1",
  cpu: "Jogador 2",
};
...
function periodName(state: BotaoMatchState) {
  ...
  if (state.period > regulation) {
    return state.setup.rules.extraHalves > 1
      ? `Prorrogação ${state.period - regulation}`
      : "Prorrogação";
  }
...
export default function BotaoMatch({
  ...
  const showFlash = useCallback(
    (text: string, tone: "goal" | "info" | "bad", duration = 1200) => {
  ...
  const chooseFormerClubCelebration = useCallback(
    (choice: "celebrate" | "respect") => {
      ...
      if (choice === "celebrate") {
        setAnnouncement(
          `Você comemorou o gol contra o ${clubName}. A reação das arquibancadas mudou na hora.`,
        );
        showFlash("COMEMOROU!", "goal", 1050);
      } else {
        setAnnouncement(
          `Você segurou a comemoração contra o ${clubName} em respeito à sua história no clube.`,
        );
  ...
  useEffect(() => {
    ...
    const syncOrientation = () => {
      setDesktopLandscape(
        desktop.matches &&
          window.localStorage.getItem("futbobo_botao_landscape") === "1",
      );
    };
    syncOrientation();
      ...
      for (const event of events) {
        if (event.type === "hit") {
          ...
          if (event.strength > 0.14 && now - lastHitRef.current > 70) {
            ...
            if (event.strength > 0.25)
              vibrate(Math.round(6 + event.strength * 22));
          }
        }
        if (event.type === "post") {
          playBotaoSound("post");
        ...
        if (event.type === "goal") {
          ...
          const finalFrame = replayFrame(
            machine,
            replayNow - replayShotStartedAtRef.current,
          );
          const currentTurn = [...replayBufferRef.current, finalFrame]
            .filter(
              (frame, index, list) =>
                index === 0 || frame.at > list[index - 1].at,
            )
            .slice(-REPLAY_MAX_FRAMES_PER_TURN);
          const turns = [
            ...replayPreviousTurnsRef.current.slice(-(REPLAY_MAX_TURNS - 1)),
            currentTurn,
          ...
          if (!localMatch && mine && event.byUser && setup.formerClub) {
            ...
            setFormerClubGoalPrompt({
              goalNumber: formerClubCelebrationsRef.current.length + 1,
            });
          }
        }
      }
  ...
  useEffect(() => {
    let frame = requestAnimationFrame(function loop(time: number) {
      ...
      if (!pausedNow && (state.phase === "aim" || state.phase === "kickoff")) {
        const clockEvents = advanceClock(
          state,
          elapsed,
          pointerRef.current !== null,
        );
      ...
      if (state.finalShotGrace && !finalShotAnnouncedRef.current) {
        ...
        setAnnouncement(
          "O tempo acabou com o chute armado. Solte: a partida só termina quando a bola parar.",
        );
        vibrate([0, 45, 35, 90]);
      ...
      if (!waitingForUser || state.finalShotGrace) {
        ...
        if (idleRemainingRef.current === null)
          idleRemainingRef.current = USER_DECISION_SECONDS;
        idleRemainingRef.current = Math.max(
          0,
          idleRemainingRef.current - elapsed,
        );
        const remaining = idleRemainingRef.current;
        const nextCountdown =
          remaining <= USER_WARNING_SECONDS
      ...
      if (!pausedNow) {
        ...
        if (resolvingForReplay) {
          ...
          if (
            replayLastSampleRef.current === 0 ||
            time - replayLastSampleRef.current >= REPLAY_SAMPLE_MS
          ) {
            replayBufferRef.current.push(
              replayFrame(state, time - replayShotStartedAtRef.current),
            );
            while (replayBufferRef.current.length > REPLAY_MAX_FRAMES_PER_TURN)
          ...
          if (replayGoalCapturedRef.current) {
            ...
            replayPreviousTurnsRef.current.push(
              replayBufferRef.current.map((frame) => ({ ...frame })),
            );
            while (
              replayPreviousTurnsRef.current.length >
              REPLAY_MAX_TURNS - 1
            ) {
        ...
        if (
          ...
          (state.phase === "penalties" && state.penalties?.shotInFlight)
        ) {
        ...
        if (goalFlashRef.current > 0) {
          goalFlashRef.current = Math.max(
            0,
            goalFlashRef.current - elapsed * 1.6,
          );
        }
        ...
        if (state.phase === "resolving") {
          ...
          if (events.length > 0 || frameCountRef.current % 6 === 0)
            handleEvents(events);
      ...
      if (desktopLandscape) {
        context.setTransform(
          ...
          -scale,
          0,
          offsetX + (VIEW_HEIGHT - VIEW_PAD_Y) * scale,
          offsetY + VIEW_PAD_X * scale,
        );
      } else {
        context.setTransform(
          scale,
          ...
          0,
          scale,
          offsetX + VIEW_PAD_X * scale,
          offsetY + VIEW_PAD_Y * scale,
        );
      }
      drawMatch(
        context,
  ...
  useEffect(() => {
    ...
    const resumeGoal = () => {
      ...
      if (current.phase !== "finished") {
        ...
        showFlash(
          `Formação ${formation.name} · ${formation.shape}`,
          "info",
          1400,
        );
      }
  ...
  useEffect(() => {
    ...
    const penalty =
      ...
      state.penalties?.turn === "cpu" &&
      !state.penalties.shotInFlight;
    ...
    const playCpuTurn = () => {
      ...
      if (
        current.phase === "penalties" &&
        current.penalties &&
        !current.penalties.shotInFlight
      ) {
        ...
        if (
          beginPenaltyShot(
            ...
            chooseCpuPenaltyShot(
              ...
              shooter,
              keeper,
              cpuSkillFor(current.setup.cpuTeam.strength),
              current.rng,
            ),
          )
        ) {
        ...
        if (current.setup.managerMode)
          chooseCpuSubstitution(current, "cpu", current.rng);
  ...
  useEffect(() => {
    ...
    showFlash(
      localMatch ? "FIM DE JOGO" : won ? victoryLabel : "FIM DE JOGO",
      localMatch || won ? "goal" : "bad",
      2400,
    );
    ...
    (x: number, y: number) => {
      ...
      if (state.phase === "penalties") {
        if (
          !state.penalties ||
          state.penalties.shotInFlight ||
          (!localMatch && state.penalties.turn !== "user")
        )
          return null;
        const shooter = penaltyShooter(state);
        return Math.hypot(shooter.x - x, shooter.y - y) <= shooter.radius + 16
          ? shooter
          : null;
      }
      ...
      for (const body of state.bodies) {
        ...
        if (distance <= body.radius + 12 && (!best || distance < best.distance))
  ...
  const roleLabel = player
    ? slotRoleLabel(userFormation.slots[player.slot] ?? userFormation.slots[0])
    : "";
  const yourTurn =
    (state.phase === "aim" || state.phase === "kickoff") &&
    state.turn === "user";
    ...
    >
      <header className="botao-hud">
        <div className="botao-hud-top">
          ...
          <span
            className={`botao-period ${!penalties && state.clock <= 15 && state.phase !== "finished" ? "botao-period-urgent" : ""} ${state.finalShotGrace ? "botao-period-final-shot" : ""}`}
          >
            {penalties
        ...
        {penalties && state.penaltyReason === "inactivity" ? (
          ...
          <div className="botao-penalty-track">
            {SIDES.map((side) => (
              <div key={side} className="botao-penalty-row">
                <span>
                  {side === "user" ? setup.userTeam.abbr : setup.cpuTeam.abbr}
                </span>
                <div className="botao-penalty-dots">
                  {Array.from({
                    length: Math.max(
                      setup.rules.penaltyRounds,
                      penalties.results[side].length,
                    ),
                  }).map((_, index) => {
          ...
          <div className="botao-formation-row">
            <span
              className={`botao-chip ${localMatch ? "botao-chip-local" : ""}`}
            >
              {localMatch ? (
                <>
                  X1 LOCAL ·{" "}
                  <span className="botao-pointer-desktop">UM MOUSE</span>
                  <span className="botao-pointer-touch">TOQUE NA TELA</span>
                </>
              ) : (
            ...
            {localMatch ? (
              <span
                className={`botao-chip botao-local-turn-chip botao-local-turn-${activeLocalSide}`}
              >
                {activeLocalName} ·{" "}
                {activeLocalSide === "user"
                  ? setup.userTeam.abbr
                  : setup.cpuTeam.abbr}
              </span>
            ) : player ? (
      ...
      {formerClubGoalPrompt && setup.formerClub && (
        <div
          className="botao-former-club-decision"
          role="dialog"
          aria-modal="true"
          aria-label="Comemoração contra ex-clube"
        >
          <div className="botao-former-club-card">
            ...
            <p>
              Você passou por esse clube. O estádio está olhando para a sua
              reação.
            </p>
            <div className="botao-former-club-actions">
              <button
                type="button"
                className="botao-former-celebrate"
                onClick={() => chooseFormerClubCelebration("celebrate")}
              >
                <b>🔥 Comemorar</b>
              ...
              <button
                type="button"
                className="botao-former-respect"
                onClick={() => chooseFormerClubCelebration("respect")}
              >
                <b>🤝 Não comemorar</b>
      ...
      >
        ...
        {!localMatch && !paused && idleCountdown !== null ? (
          <div
            className="botao-inactivity-countdown"
            role="alert"
            aria-live="assertive"
          >
            <div>
        ...
        {paused ? (
          <div className="botao-pause-overlay" role="status" aria-live="polite">
            <span className="botao-pause-mark" aria-hidden="true">
              II
            </span>
            ...
            <p>
              {localMatch
                ? "Relógio e bola estão congelados."
                : "Relógio da partida, bola, adversário e regra dos 7s estão congelados."}
            </p>
            <button
              type="button"
        ...
        {!paused && state.phase === "penalty-setup" && penalties ? (
          <div className="botao-overlay">
            ...
            {localMatch ? (
              <p>
                Os jogadores alternam as cobranças com o mesmo mouse.{" "}
                <b>{playerNames.user}</b> começa.
              </p>
            ) : (
              <>
                <p>
                  ...
                  <b>
                    {player ? `${player.label} (#${player.number})` : "você"}
                  </b>{" "}
                  vai bater?
                </p>
                <div className="botao-round-picker">
                  {Array.from({
                    length: Math.min(setup.rules.penaltyRounds, 5),
                  }).map((_, index) => (
                    <button
                      key={index}
                      type="button"
                    ...
                    >
                      {index + 1}ª
                      {index + 1 === setup.rules.penaltyRounds ? (
                        <small>decisiva</small>
                      ) : null}
                    </button>
                  ))}
                </div>
        ...
        {substitutionOpen && managerRoster ? (
          <div
            className="botao-substitution-overlay"
            role="dialog"
            aria-modal="true"
      ...
      <footer className="botao-controls">
        {paused ? (
          <p className="botao-turn botao-turn-paused">
            Jogo congelado · retome quando estiver pronto
          </p>
        ) : state.phase === "penalty-setup" ? (
          <p className="botao-turn">
            {localMatch ? "Preparem as cobranças" : "Escolha a sua cobrança"}
          </p>
        ) : penalties ? (
          <p
            className={`botao-turn ${localMatch || penalties.turn === "user" ? "botao-turn-active" : ""}`}
          >
            {localMatch
          ...
          (state.phase === "aim" || state.phase === "kickoff") ? (
          <p
            className={`botao-turn botao-turn-active botao-turn-local botao-turn-local-${activeLocalSide}`}
          >
            <b>{activeLocalName}</b> · passe o mouse e faça seu toque
          </p>
        ) : state.finalShotGrace ? (
          <p className="botao-turn botao-turn-active botao-turn-final-shot">
            00:00 · ÚLTIMO LANCE — solte e deixe a bola decidir
          </p>
        ) : yourTurn ? (
          <p className="botao-turn botao-turn-active">
          ...
          <p className="botao-turn">
            {cpuThinking
              ? `${setup.cpuTeam.shortName} está pensando…`
              : "Vez do adversário"}
          </p>
        )}
