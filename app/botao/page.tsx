...
function characterPlayersEnabled() {
  ...
  try {
    return (
      JSON.parse(window.localStorage.getItem("futbobo-settings") ?? "{}")
        .characterButtonsEnabled !== false
    );
  } catch {
    return true;
  }
...
export default function BotaoStandalonePage() {
  const clubs = useMemo(
    () => CLUBS.slice().sort((a, b) => a.name.localeCompare(b.name, "pt-BR")),
    [],
  );
  const defaultClub = useMemo(
    () => clubs.find((club) => club.id === "palmeiras") ?? clubs[0],
    [clubs],
  );
  ...
  const club =
    clubs.find((candidate) => candidate.id === clubId) ?? defaultClub;
        ...
        <div className="botao-card">
          ...
          {matchGoals.length > 0 ? (
            <div className="botao-result-lines">
              {matchGoals.map(({ entry, timelineIndex }) => {
                const replayIndex =
                  result.replays?.findIndex(
                    (replay) => replay.timelineIndex === timelineIndex,
                  ) ?? -1;
                return (
                  <div
                    key={timelineIndex}
                    className={`botao-result-line ${entry.byUser ? "botao-result-line-you" : ""}`}
                  >
                    <span className="botao-result-goal-copy">
                      <b>{entry.text}</b>
                      <span>
                        {entry.side === "user"
                          ? setup.userTeam.abbr
                          : setup.cpuTeam.abbr}{" "}
                        · {formatGoalMinute(entry, setup.rules)}
                      </span>
                    </span>
                    {replayIndex >= 0 && (
                      <button
                        ...
                        onClick={() =>
                          setActiveReplay(
                            activeReplay === replayIndex ? null : replayIndex,
                          )
                        }
                      >
                        {activeReplay === replayIndex
                          ? "Fechar replay"
                          : "Ver replay"}
                      </button>
                    )}
          ...
          {activeReplay !== null && result.replays?.[activeReplay] && (
            <GoalReplay
              ...
              setup={setup}
              label={
                result.timeline[result.replays[activeReplay].timelineIndex]
                  ?.text ?? "Gol da partida"
              }
            />
          )}
  ...
  return (
    <main className="botao-lobby">
      ...
      <p className="botao-lobby-lead">
        Cinco botões de cada lado, um toque por vez. Você é um dos botões — e a
        posição da sua carreira decide onde você começa. A cada gol os dois
        times trocam de formação.
      </p>
      ...
      <div className="botao-card">
        ...
        <div className="botao-field-row">
          <label htmlFor="botao-opponent">
            Adversário — {clubSubtitle(opponent)}
          </label>
          <select
            id="botao-opponent"
        ...
        <div className="botao-formation-row">
          <span className="botao-chip">
            Dificuldade {makeSetup().difficulty}/5
          </span>
          <button
            type="button"
