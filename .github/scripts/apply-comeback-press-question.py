...
function_anchor = "export function buildPressConference(state: GameState, match: PendingBotaoMatch, result: BotaoMatchResult, opponentName: string): PressConference {"
if function_anchor not in source:
    raise SystemExit("buildPressConference anchor not found")

addition = '''const COMEBACK_ANSWERS: AnswerDraft[] = [
...
old = '''  const questionCount = 1 + Math.floor(seeded(state.seed, match.season * 2111 + match.id.length) * 3);
  return {
    kind: "post-match",
    matchId: match.id,
...
'''
new = '''  const questionCount = 1 + Math.floor(seeded(state.seed, match.season * 2111 + match.id.length) * 3);
...
if old not in source:
    raise SystemExit("question return block not found")
